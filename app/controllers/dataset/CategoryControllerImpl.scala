...
protected[controllers] class CategoryControllerImpl @Inject() (
  ...
  protected lazy val dataSetName = result(dsa.metaInfo, 120000 millis).name
  ...
  override protected def editView(id: BSONObjectID, f : Form[Category])(implicit msg: Messages, request: RequestHeader) =
    category.edit(dataSetName + " Category", id, f, allCategories, router)
  ...
  override protected def listView(page: Page[Category])(implicit msg: Messages, request: RequestHeader) = {
    ...
    category.list(
      ...
      page,
      router,
      result(futureMetaInfos, 120000 millis)
    )
  }
  ...
  protected def allCategories = {
    // TODO: following sort throw an exception if more than 50 categories
//    val categoriesFuture = categoryRepo.find(None, Some(Seq(AscSort("name"))))
    val categoriesFuture = categoryRepo.find().map(_.toSeq.sortBy(_.name))
    result(categoriesFuture, 120000 millis)
  }
