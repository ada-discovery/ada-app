...
class DataSpaceMetaInfoController @Inject() (
  ...
  ) extends CrudController[DataSpaceMetaInfo, BSONObjectID](repo) {
  ...
  override protected def createView(f : Form[DataSpaceMetaInfo])(implicit msg: Messages, request: RequestHeader) =
    html.dataspace.create(f)

  override protected def showView(id: BSONObjectID, f: Form[DataSpaceMetaInfo])(implicit msg: Messages, request: RequestHeader) = {
    html.dataspace.show(f.value.get, result(getDataSetSizes(f.value.get)), result(repo.find()))
  }

  override protected def editView(id: BSONObjectID, f: Form[DataSpaceMetaInfo])(implicit msg: Messages, request: RequestHeader) = {
    html.dataspace.edit(id, f, result(getDataSetSizes(f.value.get)), result(repo.find()))
  }

  override protected def listView(currentPage: Page[DataSpaceMetaInfo])(implicit msg: Messages, request: RequestHeader) =
