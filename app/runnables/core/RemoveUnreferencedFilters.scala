  ...
  override def runAsFuture(
    ...
    for {
      ...
      _ <- {
        ...
        val refFilterIds2 = classificationResults.flatMap(result => Seq(result.setting.filterId, result.setting.replicationFilterId).flatten).toSet
        val refFilterIds3 = regressionResults.flatMap(result => Seq(result.setting.filterId, result.setting.replicationFilterId).flatten).toSet
