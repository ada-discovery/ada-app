...
def getIdByRole(existingRequest: BatchOrderRequest, user: Option[User])= {
  ...
  for{
    ...
    requesterId = Traversable(existingRequest.createdById.getOrElse(throw  new AdaException(("no requester id found for request: "+ existingRequest._id.get))))
    ownerIds <-   dataSetSettingRepo.find(Seq("dataSetId" #== existingRequest.dataSetId)).map{ dataSets => dataSets.filter(dataSet => dataSet.ownerId.isDefined).map(dataSet => dataSet.ownerId.get) }
  }yield
    {
