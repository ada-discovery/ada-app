          ...
          if (selectedIds.isEmpty) {
            ...
            for {
              ...
              val notification = NotificationInfo(
                ...
                possibleActions =  ActionGraph.asMap.get(createAction.toState).map(_.map(_.action)),
        ...
        notifications = roleUsersToNotify.flatten { case (role, users) =>
          users.map { user =>
            ...
            NotificationInfo(
              ...
              possibleActions =  ActionGraph.asMap.get(allowedStateAction.toState).map(_.map(_.action)),
              createdByUser = roleUsersToNotify(Role.Requester).toSeq.head.ldapDn,
              targetUser = user.ldapDn,
